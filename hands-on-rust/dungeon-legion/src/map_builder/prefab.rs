...
pub fn apply_prefab(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    if let Some(placement) = placement {
        println!("Fortress at: {:?}", placement);
        println!("Player at: {:?}", mb.player_start);
        ...
        for ty in placement.y..placement.y + FORTRESS.2 {
            for tx in placement.x..placement.x + FORTRESS.1 {
                ...
                // let new_y: usize = (ty - placement.y + 1).try_into().unwrap();
                // let new_x: usize = (tx - placement.x).try_into().unwrap();
                // let new_i: usize = new_y * new_x;
                // let c = string_vec[new_i];
                let c = string_vec[i];
